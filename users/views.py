...
import os
from .placement import place_fragments, replicate_fragments
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            path = os.path.join(settings.MEDIA_ROOT,
                                str(file_to_split.file_name))
            splitted_file_path = split_file(path, number_of_chunks)
            if splitted_file_path:
                stored_nodes = place_fragments(
                    splitted_file_path, [0, 1, 2, 3])
                nodes = [node.split("/")[-1] for node in stored_nodes]
                replicated_chunks_nodes = replicate_fragments(
                    splitted_file_path, [0, 1, 2, 3], nodes)
                replicated_nodes = [node.split("/")[-1]
                                    for node in replicated_chunks_nodes]
                file_to_split.nodes = nodes
                file_to_split.replicated_nodes = replicated_nodes
                file_to_split.save()
        return redirect('file_list')
    form = SplitForm()
